...
class ConnectCreate extends Form {
  ...
  onTypeChange = ({ currentTarget: input }) => {
    ...
    this.setState({ selectedType: value },()=>{ this.renderForm()});
  ...
  handleGroup(group) {
    let groupDisplay = [<label style={{ backgroundColor: 'blue' }}>{group[0].group}</label>];
    group.map(element => {
      const label = this.renderLabel(element);
      const name = element.name;
      groupDisplay.push(this.renderInput(name, label, ''));
    });
    return groupDisplay;
  }
